...
public class PerceptiveHash implements Comparer {
    @Override
    public CompareResult compareImages(PictureHolder bHolder, PictureHolder cHolder, String resultFile, ElementStatus elementStatus, BufferedImage siteFullpage,BufferedImage fullPageChangesScreenshot,HashMap<String,String> config) {
        ...
        HashingAlgorithm hasher = new com.github.kilianB.hashAlgorithms.PerceptiveHash(64);
        ...
        if(similarityScore > 0.1){
            compareResult.getScorePriority().put("perceptiveHashScore", "Changed" );
        }else {
            compareResult.getScorePriority().put("perceptiveHashScore", "Same");
        }
        compareResult.getScore().put("perceptiveHashScore", String.valueOf(similarityScore));
